...
namespace WebShop
{
    public abstract class BaseManager<T> //bazes klase- no tas nav paredzets veidot mainigos. klase no kuras atsaukties citam klasem
        //Base manager ir jaizsauc noradot tipu kads bus nepieciesams. T ir tipa mainigais . T ir generic type, kas lauj veidot universalas klases
         
        where T : BaseData //so japieliek, ja nu norada datubazei nederigu mainigo- ne klasi. un : BaseData norada ka jaatbilst basedata klasei

    {
        protected WebShopDB _db; //protected starp prvatu un publisku. pieejams sai un klases kas manto so klasi
